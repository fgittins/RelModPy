...
class TestEnergyPolytrope(TestCase):
    ...
    def test_interpolation(self):
        ...
        np.testing.assert_allclose(
                m(self.polystar.rsol), self.polystar.msol)
        np.testing.assert_allclose(
                p(self.polystar.rsol), self.polystar.psol)
        np.testing.assert_allclose(
                nu(self.polystar.rsol), self.polystar.nusol)

    def test_surface(self):
        """Test that pressure vanishes at surface."""
        np.testing.assert_allclose(
                self.polystar.psol[-1], 0, rtol=0, atol=1e-15)

    def test_stellar_mass(self):
        R = 6.465
        M = R/2*0.594
        np.testing.assert_allclose(
                self.polystar.M, M, rtol=0, atol=1e-3)

    def test_stellar_radius(self):
        R = 6.465
        np.testing.assert_allclose(
                self.polystar.R, R, rtol=0, atol=1e-3)

    def test_monotonicity(self):
        """Test that integration results are monotonic."""
        np.testing.assert_equal(
                np.diff(self.polystar.psol) <= 0, True)
        np.testing.assert_equal(
                np.diff(self.polystar.msol) >= 0, True)
        np.testing.assert_equal(
                np.diff(self.polystar.nusol) >= 0, True)
        ...
        for K in self.Krange:
            for pc in self.pcrange:
                with self.subTest(K=K, pc=pc):
                    ...
                    np.testing.assert_allclose(
                            m(polystar.rsol), polystar.msol)
                    np.testing.assert_allclose(
                            p(polystar.rsol), polystar.psol)
                    np.testing.assert_allclose(
                            nu(polystar.rsol), polystar.nusol)
        ...
        for K in self.Krange:
            for pc in self.pcrange:
                with self.subTest(K=K, pc=pc):
                    ...
                    np.testing.assert_allclose(
                            polystar.psol[-1], 0, rtol=0, atol=1e-15)
        ...
        for K in self.Krange:
            for pc in self.pcrange:
                with self.subTest(K=K, pc=pc):
                    ...
                    np.testing.assert_equal(
                            np.diff(polystar.psol) <= 0, True)
                    np.testing.assert_equal(
                            np.diff(polystar.msol) >= 0, True)
                    np.testing.assert_equal(
                            np.diff(polystar.nusol) >= 0, True)
